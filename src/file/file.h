 ...
 *
 * 当调用 dup(), dup2() 或 fcntl() 为一文件描述符创建副本时,
 * 总是会清除 `FD_CLOEXEC` 标志位.
 *
 ...
 * `close-on-exec` 标志位, 则在执行 execve() 系列函数时, 新程序将关闭该文件描述符.
 *
 * @return 函数执行成功返回复制出来的文件描述符
 * @retval -1 函数执行失败
 *
 ...
 *   新程序将关闭该文件描述符. 若没有指定该参数, 新程序中会对该文件描述符保持打开状态.
 *   如果 execv() 系列函数执行失败, 则不会关闭文件描述符.
